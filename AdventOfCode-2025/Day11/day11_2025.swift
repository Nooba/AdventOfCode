...
func day11_2025_A() throws -> Int {
    let lines = try FileReader(filename: "day11_2025_input").getLines()
...
private func blinkBuckets() {
    ...
    buckets.forEach { (key: Int, value: Int) in
        ...
        nextBuckets[key * 2025] = (nextBuckets[key * 2025] ?? 0) + value
    }
    buckets = nextBuckets
}
