...
private extension Map {
    func computeProperCosts() -> Int {
        ...
        self[findCell(value: .exit)]!.orientation = nil
        return self[findCell(value: .exit)]!.cost
    }
...
private extension Map {
    func computeBestSeats() -> Int {
        var cellsToPropagateFrom = [(findCell(value: .exit), self[findCell(value: .exit)]!.cost)]
        while !cellsToPropagateFrom.isEmpty {
            let nextCells = cellsToPropagateFrom.flatMap { (cell, cost) in
                propagateBestSeat(from: cell, cost: cost)
            }
            cellsToPropagateFrom = nextCells
            print(self)
        }
        return rows.compactMap { row in
            return row.filter { $0.isBestSeat }.count
        }.reduce(0, +)
    }

    private func propagateBestSeat(from origin: Position, cost: Int) -> [(Position, Int)] {
        guard let cell = self[origin] else { return [] }
//        guard let originOrientation = cell.orientation else {
//            return []
//        }
        var nextCells = [(Position, Int)]()
        Direction.allCases.forEach { direction in
//            guard direction != originOrientation else { return }
            switch direction {
            case .left:
                if let leftCell = self[origin.left], !leftCell.isWall {
                    if leftCell.cost < cost {
                        leftCell.isBestSeat = true
                        nextCells.append((origin.left, cost - costBetween(origin: leftCell, target: cell)))
                    }
                }
            case .right:
                if let rightCell = self[origin.right], !rightCell.isWall {
                    if rightCell.cost < cost {
                        rightCell.isBestSeat = true
                        nextCells.append((origin.right, cost - costBetween(origin: rightCell, target: cell)))
                    }
                }
            case .up:
                if let upCell = self[origin.above], !upCell.isWall {
                    if upCell.cost < cost {
                        upCell.isBestSeat = true
                        nextCells.append((origin.above, cost - costBetween(origin: upCell, target: cell)))
                    }
                }
            case .down:
                if let downCell = self[origin.below], !downCell.isWall {
                    if downCell.cost < cost {
                        downCell.isBestSeat = true
                        nextCells.append((origin.below, cost - costBetween(origin: downCell, target: cell)))
                    }
                }
            }
        }
    ...
    private func costBetween(origin: Cell, target: Cell) -> Int {
        guard let originOrientation = origin.orientation, let targetOrientation = target.orientation else { return 1 }
...
func day16_2024_B() throws -> Int {
    let lines = try FileReader(filename: "day16_2024_example").getLines()
    let map = try parseMap(lines)
    let result = map.computeProperCosts()
    return map.computeBestSeats()
}
